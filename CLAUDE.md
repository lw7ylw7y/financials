...
| `GITHUB_MODELS_TOKEN` | Optional fallback for the AI calls (`interpret.py`, `ticker_valuation.py`): when a Gemini call fails, the same prompt is retried against a second Gemini model (`gemini-3.8-flash-lite` by default, override with `GEMINI_FALLBACK_MODEL`; same `GEMINI_API_KEY`), then `web/github_models_client.py` against GitHub Models (`openai/gpt-4.1-mini` by default, override with `GITHUB_MODELS_MODEL`). A fine-grained PAT with the `models` permission for a hosted web app; the scheduled workflow passes `secrets.GITHUB_TOKEN` (needs `permissions: models: read`, already set). Falls back to `GITHUB_TOKEN` if unset; with neither, the fallback is skipped and behavior is as before. An explicitly injected Gemini `client` (tests) never falls back |
