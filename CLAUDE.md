...
| `ANTHROPIC_API_KEY` | Optional last-resort fallback for the AI calls (`interpret.py`, `ticker_valuation.py`): when a Gemini call fails, the same prompt is retried on a second Gemini model (`gemini-3.7-flash` by default, override with `GEMINI_FALLBACK_MODEL`; same `GEMINI_API_KEY`), then on Claude via `web/claude_client.py` (`claude-haiku-4-5-20251001` by default, override with `CLAUDE_FALLBACK_MODEL`). Needs an Anthropic Console API key — API usage is billed per use, separately from a claude.ai subscription. With no key the Claude step is skipped and behavior is as before. An explicitly injected Gemini `client` (tests) never falls back. Needed on Render/local `.env` and, as `ANTHROPIC_API_KEY`, in GitHub Actions' repo secrets |
