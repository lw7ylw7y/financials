...
| AI interpretation of indicators | Gemini API (free tier), falling back to a second Gemini model, then GitHub Models | Plain-English summary + directional read; framed as informational commentary, not advice |
...
- **AI interpretation:** Gemini API (`gemini-3.8-flash`, free tier); on any failure, a second Gemini model, then GitHub Models (free tier), before degrading to no AI section / the last cached result
