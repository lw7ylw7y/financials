...
- **AI valuation section**, shown once at the top of the page (above market news): one batched AI call reasoning across the whole watchlist judges each ETF group as discount/fair/overpriced (sorted most-attractive-first) and gives every individual stock its own discount/fair/overpriced verdict with a one-sentence reasoning, grouped under three "Discount"/"Fair"/"Overpriced" headings rather than one flat list. Uses the sector-peer P/E benchmark and growth/quality stats above to distinguish a real discount from a value trap (e.g. a low P/E paired with shrinking revenue or high debt). One call for the whole watchlist, not one per ticker — Gemini's free tier caps at 20 requests/day total, shared with the indicator digest's own AI calls. Refreshes on a multi-hour throttle, not on every page load; any failure (including a quota error) falls back to the last successfully generated valuation, shown as-is rather than as an error — same graceful-degradation pattern as a ticker's own stale price
