...
- **Table, not per-ticker cards** (revised after initial use — a card grid was hard to scan across many tickers). One row per ticker, grouped under the same asset-type headers, columns: Ticker, Price, Change, 52-Week Range, % Off High, 20-day MA, 50-day MA, 200-day MA
...
- **Percent off 52-week high** (added after using the dashboard live) — its own column, showing how far below its 52-week high the current price sits, as a percentage. A precise complement to the range bar's visual read: the bar gives an at-a-glance sense of where in its range a ticker sits, this gives the exact number for "how much of a discount from its peak is this," which is what actually drives a buy decision
- **Change since last close** (added after using the dashboard live) — its own column, the current session's price move versus the previous close (absolute and %), colored with the same green-up/red-down convention as the moving-average deltas. Needed to gauge how "stale" or noisy today's price read is before weighing it against the 52-week range/moving averages — a ticker that moved 5% today reads very differently than one that's flat
- **Top-discount row highlight** — within each group, roughly the top quarter of tickers (rounded up, minimum 1) by how far below their 52-week high they sit get a faint green row background. Ranked per group, not across the whole page
...
| *(v1.1, ticker dashboard)* Moving averages (20d/50d/200d) | Yahoo Finance (public chart endpoint) | Finnhub's free tier has no daily-close source at all; Yahoo's undocumented chart endpoint is free, unauthenticated, and (unlike stooq.io, tried first) doesn't gate requests behind a bot challenge |
| *(v1.1, ticker dashboard)* Percent off 52-week high | *(computed, no new source)* | Derived from price + 52-wk range, both already fetched from Finnhub — `(week52_high - price) / week52_high` |
| *(v1.1, ticker dashboard)* Change since last close | Finnhub | Confirmed already returned by the existing quote call (`d`/`dp` fields) — no new endpoint needed |
| *(v1.1, ticker dashboard)* US stock market news | Finnhub | `/news?category=general` — same provider as quote/52-wk-range, separate from the per-ticker `/company-news` endpoint that was tried and removed. Turned out to be a broad news wire rather than market-specific; filtered client-side to Finnhub's own `"top news"` category tag as the closest available signal |
