...
- The ticker watchlist (`config/tickers.json`), the two local snapshot caches (`data/tickers.json`, `data/market_news.json`), and — as of Story 11 — indicator state (`data/indicators.json`) all move to a small free external key-value store when hosted, since a free host's filesystem doesn't persist across restarts — without this, an in-app ticker edit, a cached snapshot, or a live-refreshed AI response would silently vanish the next time the app restarts. Local development keeps using the plain JSON files as today
...
| *(v2.1, hosted dashboard)* Persistent ticker config + caches + indicator state when hosted | Upstash Redis (REST API) | Free tier, no auto-pause; stores `config/tickers.json`'s content, the two ticker/news snapshot caches, and (Story 11) `data/indicators.json`'s state as JSON blobs, since the host has no persistent disk. Read/written directly by both the scheduled GitHub Actions workflow and the hosted app — same shared store, not two separate copies |
