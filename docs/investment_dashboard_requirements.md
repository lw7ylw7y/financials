...
- Refreshable: reloading the page triggers a live data pull (not just a re-read of the last stored snapshot), so you can check in between digest emails and see genuinely current data
- Fallback: if the live pull fails (source API down, rate-limited, etc.), the page falls back to the last stored data rather than erroring out
- The page always shows a timestamp indicating whether what's displayed is **live** (just pulled) or **saved** (fallback to the stored snapshot, with the time that snapshot was recorded) — so it's never ambiguous how current the data is
- Backed by the same historical data store the v1 email pipeline already writes to for the fallback path and for history; a successful live pull should update that store too, so the email pipeline and the page stay consistent
...
- A live pull on every reload means FRED/Gemini API calls happen on a per-visit basis rather than only on the fixed ingestion schedule — worth checking this stays comfortably within free-tier rate limits given single-user, low-frequency usage
