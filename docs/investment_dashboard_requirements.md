...
- **AI valuation section**, shown once at the top of the page (above market news): one batched AI call reasoning across the whole watchlist judges each group as discount/fair/overpriced (sorted most-attractive-first) and gives every ticker, ETFs and individual stocks alike, its own discount/fair/overpriced verdict with a one-sentence reasoning (each tagged with its group), grouped under three "Discount"/"Fair"/"Overpriced" headings rather than one flat list. Also given the indicator digest's saved economic read as background. Uses the sector-peer P/E benchmark and growth/quality stats above to distinguish a real discount from a value trap (e.g. a low P/E paired with shrinking revenue or high debt). One call for the whole watchlist, not one per ticker — Gemini's free tier caps at 20 requests/day total, shared with the indicator digest's own AI calls. Refreshes on a multi-hour throttle, not on every page load; any failure (including a quota error) falls back to the last successfully generated valuation, shown as-is rather than as an error — same graceful-degradation pattern as a ticker's own stale price
...
- **Sign-in-protected**, not public: every route requires signing in with the one allowed Google account before rendering anything. No user accounts or roles exist beyond that single account.
...
- **Historical P/E/PEG for a ticker's own valuation trend** — neither Finnhub nor Yahoo offers this for free (only point-in-time snapshots); would need this app to start persisting its own `pe_ratio`/`peg_ratio` readings over time, the same append-only pattern already used for the 8 macro indicators, and would only produce useful signal after months of accumulated history. The AI valuation section (Section 4.2) currently reasons only from a sector-peer benchmark and growth/quality stats, not the ticker's own trend
- **Analyst consensus in the AI valuation section** — Finnhub's `/stock/recommendation` (buy/hold/sell trends) is free-tier and confirmed working but unused; `/stock/price-target` is paid-tier only (confirmed 403), so a price-target-based read isn't available at all
...
| AI interpretation of indicators | Gemini API (free tier), falling back through several other Gemini models | Plain-English summary + directional read; framed as informational commentary, not advice |
...
- **AI interpretation:** Gemini API (`gemini-3.8-flash`, free tier); on any failure, each of several fallback Gemini models in turn, before degrading to no AI section / the last cached result
