...
- `last_digest_sent_at` (top-level, alongside `indicators`) is the ISO timestamp of the last digest email actually sent — absent/null until the first digest ever fires. `post_release.indicators_updated_since` compares it against each indicator's latest `fetched_at` to name what's new in the email's subject when one does send; `post_release.maybe_send_digest_email` also uses it to enforce the weekly send interval (Section 5). `last_digest_content_fingerprint` (top-level, added v2.1/Story 11) is a hash of the digest's actual conclusion as of the last send — the real send-gate now, with `last_digest_sent_at` acting only as the minimum-interval floor on top of it
- `last_ai_response` (top-level, added v2) — as of v2.1/Story 11, refreshed every ingestion cycle that finds new data (`post_release.refresh_ai_response_if_updated`), not only when an email is also about to send; see `docs/v2_technical_design.md` Section 11.7 for why this changed and Redis-backed persistence, which replaced `data/indicators.json` as a git-committed file when hosted
...
- **The AI response is refreshed every scheduled check that finds new data** (`post_release.refresh_ai_response_if_updated`, added v2.1/Story 11) — no throttle on this part; `state["last_ai_response"]` stays as fresh as the ingestion itself, whether or not an email is also due
- **The digest email is throttled separately, to a weekly rollup** (`post_release.maybe_send_digest_email`), gated on two conditions, evaluated every scheduled check regardless of whether the AI refresh above also ran this same cycle:
  1. The digest's actual content — a fingerprint of every indicator's latest value/date plus the AI's directional read (`post_release._digest_fingerprint`) — differs from what was last emailed (`last_digest_content_fingerprint`). Deliberately excludes the AI summary's free text, since it can be reworded differently between calls for an unchanged situation. This replaced an earlier "any indicator fetched since the last digest" timestamp scan (`indicators_updated_since`), which is still used, but now only to name what's new in the subject line once a send has already been decided
  ...
  - The very first digest ever (`last_digest_sent_at` unset) sends immediately once there's anything to report, without waiting a week
  - `maybe_send_digest_email` never calls the AI itself (no `interpret_fn` parameter exists on it) — it only ever reuses whatever `refresh_ai_response_if_updated` most recently persisted, so the two can never trigger a duplicate Gemini call in the same cycle
