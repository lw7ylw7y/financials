...
```mermaid
graph TD
    subgraph "Render free web service (public, HTTPS, Google sign-in-gated)"
        REQ[Any request] --> AUTH{Google sign-in<br/>GOOGLE_CLIENT_ID / ALLOWED_EMAIL}
        AUTH -->|fail| R401[redirect to /login, or 401 for /api/*]
...
- The Google OAuth client is created in the Google Auth Platform console (consent screen "External", left in Testing with the owner's account as a test user, which needs no Google verification for `openid email`). First sign-in shows Google's "unverified app" warning once.
...
- Render env vars needed, in addition to the existing `FRED_API_KEY`/`GEMINI_API_KEY`/`FINNHUB_API_KEY`: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `ALLOWED_EMAIL`, `SECRET_KEY`, `UPSTASH_REDIS_REST_URL`, `UPSTASH_REDIS_REST_TOKEN`. `GMAIL_*` are not needed on Render — the digest email still only sends from the GitHub Actions workflow
