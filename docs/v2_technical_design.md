...
### 11.8 Ticker Dashboard: per-group live checks (Story 12) — app code implemented; Render concurrency fix pending
...
**New race, fixed in the same change:** before this split, only one combined live-check request ever ran per page load, so a read-modify-write race on the shared `data/tickers.json`/Redis cache was only a *theoretical* risk (two different visitors' page loads overlapping). Splitting into 5 concurrent per-group requests from a single page load turns that into a near-certainty every time the page runs its background check: `check_for_ticker_updates` loads the full cache, merges in only its own group's fresh results, and saves the full (merged) cache back — if two groups' calls both load before either saves, the second save silently reverts the first group's freshly-fetched values back to their pre-check snapshot. Fixed by wrapping just the load-merge-save portion (not `build_ticker_cards`'s network fetch) in a new process-local `ticker_dashboard._ticker_state_lock`, so concurrent groups still fetch their live data fully in parallel and only the brief in-memory merge/write is serialized. Verified directly (not just reasoned about): running all 5 real groups (36 tickers) concurrently against the live Finnhub/Yahoo APIs and inspecting `data/tickers.json` afterward confirmed all 36 symbols survived with no data loss. Same process-local caveat as the rest of this project's locking (doesn't protect across multiple gunicorn *worker processes*, only within one) — acceptable since Render's current start command runs a single worker.

**Local dev:** `app.py`'s `if __name__ == "__main__":` block now passes `threaded=True` to `app.run(...)`, since Werkzeug's dev server is single-threaded by default and would otherwise queue the 5 concurrent per-group requests one at a time, masking the whole point of the split during local testing. Confirmed the difference directly: the same 5 real group fetches totaled ~3s wall-clock run concurrently (`threaded=True`) versus ~7.4s summed sequentially — both far below the original single-request design's occasional 30+s.

**Render gap, found live after this shipped:** the app-code half of this story (the routes, the lock, `threaded=True` locally) doesn't by itself make the split concurrent *on Render* — that also needs the host's own request-handling concurrency, which the existing gunicorn start command doesn't have (Section 11.5 predates this story and only ever needed `--timeout 120`, since the old design was one request, not several meant to run at once). Without it, gunicorn's default single sync worker queues the 6 per-page-load requests (5 groups + market news) and handles them one at a time — observed live as the fastest group returning in a couple of seconds and the slowest only after ~35s, well past what any single group's own fetch takes, since it's really waiting on every request queued ahead of it. The fix is the `--worker-class gthread --threads 8` addition documented in Section 11.5, not an app-code change — it hasn't been applied on Render yet as of this writing.
