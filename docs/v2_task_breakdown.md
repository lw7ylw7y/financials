...
| 12.6 `ticker_dashboard.py` — `_fetch_concurrency_limit`, a global `threading.Semaphore(5)` every `_build_card` call acquires, restoring the system-wide Finnhub/Yahoo concurrency cap that `build_ticker_cards`'s per-call `max_workers` alone stopped providing once multiple groups' calls could run at once | done — found live: without this, real per-group concurrency (once gunicorn threading was added) pushed global external-API concurrency past what Finnhub's free tier/a constrained host tolerates, regressing to 20-35s per group |
| 12.7 Render start command — add `--worker-class gthread --threads 8` so gunicorn can actually handle the 6 concurrent per-page-load requests (5 groups + market news) at once, not just accept them | **pending** — found live: without this, gunicorn's default single sync worker queues the requests and the slowest ends up waiting ~35s behind the others, negating the split's benefit on Render even though the app code and local dev are both correct. Threads, not `--workers N` — `_fetch_concurrency_limit` (12.6) is process-local, so separate worker processes wouldn't share it (the ticker-cache write itself, 12.4, is Redis-atomic and safe across any number of processes). Same "Save, rebuild, and deploy" gotcha as any other Render start-command/env-var change (Section 11.5) |
| 12.8 One-time Redis migration — delete the pre-existing `ticker_cache` key (a string, from the old `set_json`/`get_json` design) so it can be recreated as a hash | **pending on Render** — done against the real Upstash instance during local verification (same shared instance Render uses); confirmed a fresh check repopulates it correctly. Must happen again (or already has, incidentally) the moment this code deploys to Render — the old key's wrong type makes every `HSET`/`HGETALL` fail (`WRONGTYPE`/400) until it's deleted, degrading to an empty cache rather than crashing, but not self-healing on its own |
...
- Manual: confirmed locally (against the real, shared Upstash instance) that all 5 configured groups (36 tickers total) complete concurrently in ~3-5s total versus the original single request's occasional 30+s; confirmed no data loss in the `ticker_cache` hash after a concurrent all-groups run (every successfully-fetched symbol present; the only misses were genuine per-ticker Finnhub failures, unrelated to concurrency) — verified against the actual live race and the actual live wire format, not just in theory
